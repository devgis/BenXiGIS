 ...
 new dojo.Color([255, 0, 0]), 2), new dojo.Color([255, 255, 0, 0.5]));
    ...
    function showQueryResult(idResults) {
        ...
        if (idResults.length > 0) {
           ...
           // alert(name);
            window.open("http://jiekou.chengtianhuiju.com:9099/zhanwang/Reservoir/search.aspx?paddress=" + name, "newwindow", "height=326, width=710, top=400, left=400, toolbar=no, menubar=no, scrollbars=yes, resizable=no,location=no, status=no")
...
function searchLocalAndServer(word) {
    ...
    for (var i = 0; i < ggzLayer.graphics.length; i++) {
        ...
        Color, dom, on
          ...
          var gra = new Graphic(ggzLayer.graphics[i].geometry, markerSymbol);
          searchrsLayer.add(gra);
          //gra.attributes["NAME"] = ggzLayer.graphics[i].attributes["Name"];
          gra.setAttributes({ "NAME": ggzLayer.graphics[i].attributes["STNM"] });
